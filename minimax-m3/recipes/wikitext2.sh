...
readonly model_dir="${MODEL_DIR:?Set MODEL_DIR to the verified MiniMax-M3-NVFP4 checkpoint}"
readonly result_root="${RESULT_ROOT:-$PWD/results/minimax-m3}"
readonly output_dir="$result_root/1x-nvfp4/wikitext2-bf16-kv"
...
"$lm_eval_bin" \
  ...
  --model_args "model=MiniMax-M3-NVFP4,base_url=http://127.0.0.1:30000/v1/completions,tokenizer=$model_dir,tokenizer_backend=huggingface,tokenized_requests=True,num_concurrent=1,max_length=2048,trust_remote_code=True,timeout=900" \
  --batch_size 4 \
  --tasks wikitext \
