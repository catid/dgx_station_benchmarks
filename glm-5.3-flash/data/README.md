...
- `throughput.csv` contains three native-FP8 C1–C128 series plus current
  LibertAIDAI NVFP4/SGLang TP1 and TP2 AR and DFlash2 C1–C64 series. TP1 is one
  engine on one DGX Station; TP2 is one distributed engine across two Stations.
  DFlash2 is speculative decoding on the same NVFP4 base. Effective concurrency,
...
- `prefill.csv` contains the five measured prefill series with client rates,
  prompt lengths, sample counts, available server rates, and source hashes.
  DFlash2 changes only decode and references the matching AR prefill rather
...
- `qualification.csv` records measured lanes, pending work, the completed
  current TP1 and TP2 NVFP4 lanes, and historical attempt metadata.
- `checkpoint.json` contains audited native-FP8, current NVFP4 target, and
  DFlash2 draft/runtime facts.
- `import-nvfp4-results.py` validates and imports the completed raw NVFP4 TP1
  and TP2 AR and DFlash2 results. It checks exact target/draft/runtime/topology pins, every
...
python3 data/import-nvfp4-results.py \
  ...
  /home/catid/frontier-bench/results/glm53-tp1-sglang/glm53-nvfp4-tp1-ar-v1 \
  /home/catid/frontier-bench/results/glm53-tp1-sglang/glm53-nvfp4-tp1-dflash2-v1 \
  /home/catid/frontier-bench/results/glm53-tp1-sglang/glm53-nvfp4-tp1-dflash2-c64-v1
