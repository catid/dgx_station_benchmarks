...
# has its requirements installed). Works against either engine's OpenAI-compatible endpoint; the API address comes
# from config.env (API_URL follows SWAP_RANKS).
set -euo pipefail
...
curl -fsS --max-time 5 "$API_URL/health" >/dev/null || { echo "server not ready" >&2; exit 1; }
...
model_name="$(curl -fsS "$API_URL/v1/models" | python3 -c 'import sys,json; print(json.load(sys.stdin)["data"][0]["id"])')"
...
for c in "${concurrencies[@]}"; do
  ...
  python "$bench_root/llm_decode_bench.py" \
    --host "$client_host" --port "$API_PORT" --model "$model_name" \
