...
- Per-user decode at C1: vLLM TP2 DSpark **201.0 tok/s per user at C1** (2.56 accepted tokens per step) versus SGLang DSpark **180.0 tok/s** (2.80 accepted tokens per step), vLLM PP2 autoregressive **141.3 tok/s**, and SGLang autoregressive **106.8 tok/s**; vLLM TP2 DSpark leads every lane at every concurrency, per user and in aggregate, and nearly doubles its own AR server at C1 (102.3 tok/s).
- Peak decode: **3,401.6 aggregate tok/s** (vLLM TP2 · DSpark, C64) versus **2,805.9** for vLLM PP2 · AR (+21.2%), **2,392.3** for SGLang DSpark (+42.2%) and **2,236.7** for SGLang AR · Data Direct (+52.1%) at C64; among the autoregressive lanes PP2 leads, and SGLang DSpark still beats SGLang AR at C64 (+7.0%).
...
*Aggregate output tok/s versus concurrency, AR and DSpark on both engines; 8,192-token input, 1,024 forced output tokens, temperature 0, `C` warm-ups then `5 × C` measured requests. vLLM TP2 DSpark leads at every concurrency (+21.2% over vLLM PP2 AR at C64); vLLM TP2 AR trails PP2 AR throughout, and SGLang DSpark beats SGLang AR at every concurrency but trails vLLM PP2 AR at C4 and from C16 upward.*
...
*Median per-request output tok/s at each concurrency: vLLM TP2 DSpark leads at every concurrency (2.56 accepted tokens per step at C1); among the rest SGLang DSpark leads at C1–C2 (2.80 accepted tokens per step) and vLLM PP2 AR from C4 upward. The accept length per cell is kept in [data/throughput.csv](data/throughput.csv).*
