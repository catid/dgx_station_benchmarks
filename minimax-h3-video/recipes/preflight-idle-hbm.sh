...
readonly tolerance_mib="${IDLE_HBM_TOLERANCE_MIB:-4096}"
[[ "${EXPECTED_IDLE_HBM_MIB}" =~ ^[0-9]+$ ]] || {
  echo "EXPECTED_IDLE_HBM_MIB must be a non-negative integer" >&2
  exit 2
}
[[ "${tolerance_mib}" =~ ^[0-9]+$ ]] || {
  echo "IDLE_HBM_TOLERANCE_MIB must be a non-negative integer" >&2
  exit 2
}
...
if pgrep -af 'vllm|sglang serve|multimodal_gen|bench_serving|launch_cluster|serve_node' >&2; then
...
device="/dev/nvidia${gpu_index}"
mapfile -t owner_pids < <(
  sudo lsof -t "${device}" /dev/nvidia-uvm 2>/dev/null | sort -u || true
)
for pid in "${owner_pids[@]}"; do
  command="$(ps -p "${pid}" -o comm= | xargs)"
  if [[ "${command}" != nvidia-persiste* ]]; then
    echo "Unexpected GPU/UVM device owner: pid=${pid} command=${command}" >&2
    exit 1
  fi
done
