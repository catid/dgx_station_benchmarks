...
class SectionContractTests(unittest.TestCase):
    ...
    def test_readme_and_repository_headlines_round_from_canonical_rows(self) -> None:
        ...
        decode_table = table_after_heading(section, "## Decode throughput")
        ...
        prefill_table = table_after_heading(section, "## Cold prefill")
        ...
        self.assertIn(
            f"NVFP4 MTP3 C1: {Decimal(decode[('nvfp4_tep4_mtp3', 1)]):,.1f}",
            overview_row,
        )
        self.assertIn(
            f"NVFP4 AR C64: {Decimal(decode[('nvfp4_tep4_ar', 64)]):,.1f}",
            overview_row,
        )
        self.assertIn(
            f"FP8 AR C128: {Decimal(decode[('fp8_tep4_ar', 128)]):,.1f} tok/s",
            overview_row,
        )
    ...
    def test_local_qualification_is_current_and_unranked(self) -> None:
        ...
        self.assertEqual(
            ...
            "qwen38-sglang-nvfp4-tp1-mtp0-smoke-20260827-v15",
        )
        self.assertEqual(
    ...
    def test_no_failed_or_pending_result_is_encoded_as_zero(self) -> None:
        ...
        overlays = csv_rows("dgx-overlays.csv")
        self.assertEqual(overlays, [], "unqualified DGX timing must remain absent")
    ...
    def test_source_seals_and_mtp3_c128_exception_are_not_overstated(self) -> None:
        ...
        self.assertIn("compiled one first-use Triton kernel", section)
